//
//  UIColors+ColorPallete.swift
//  Taygır
//
//  Created by Fatih Bilgin on 18.11.2022.
//

import UIKit

// MARK: - Color Pallete

extension UIColor {
    static let light = #colorLiteral(red: 0.768627451, green: 1, blue: 0.9764705882, alpha: 1)
    static let inValidTaygir = #colorLiteral(red: 0.6199232936, green: 0.9215670824, blue: 0.8793562651, alpha: 1)
    static let taygirEx = #colorLiteral(red: 0.4784313725, green: 0.8392156863, blue: 0.9764705882, alpha: 1)
    static let taygir = #colorLiteral(red: 0.4745098054, green: 0.8392156959, blue: 0.9764705896, alpha: 1)
    static let text = #colorLiteral(red: 0.1490196078, green: 0.1490196078, blue: 0.1490196078, alpha: 1)
    static let disabledText = UIColor.text.withAlphaComponent(0.8)
}
